...
# algo interessante: enviar programaticamente comandos para o shell.
# Crie um script chamado resources.py e utilize-o para exibir no console as
# informações solicitadas abaixo. Para isso, utilize o método check_output do
# módulo subprocess do Python 😎.

# Informações sobre a sua CPU (no Linux você pode usar comando lscpu,
# e no OSX você pode usar o comando sysctl -n machdep.cpu.brand_string):
...
# A quantidade total de memória RAM disponível em sua máquina em megabytes -
# MB (faça a conversão também 😉).

# A quantidade total de memória RAM que está sendo utilizada em megabytes - MB.

# 👀 De olho na dica:

# O método decode("UTF-8") vai ser útil para ler os dados oriundos
# da check_output;

# Os métodos split e strip vão ser úteis para limpar e separar as
# informações obtidas com os comandos;

# O método startswith vai ser útil para selecionar informações específicas.

# Se estiver muito difícil fazer a filtragem e limpeza dos dados,
# se contente com a exibição de informações a mais 😉
